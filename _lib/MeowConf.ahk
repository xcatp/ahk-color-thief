...
class MeowConf {
  ...
  static AddFunc(key, fn) {
    key .= '$' (fn.IsVariadic ? '*' : fn.MaxParams)
  ...
  Init(_path, f) {
    ...
    _processValue(_l, _idx, _raw := false) {
      ...
      if _idx != 1 { ; 仅对值处理延续行
        _l := (_l.substring(1, _idx) _handleMutliLine(_l.substring(_idx)))
        cs := _l.toChs()
      }
      while _idx <= cs.Length {
        ...
        } else if !_raw and cs[_idx] = rc and !esc { ; 引用
          _i := ++_idx, _to(cs, rc, &_idx, '未找到成对的引用符'), _k := _l.substring(_i, _idx)
          ; if is func
          if _k ~= '^\w+\(.*?\)$' {
            DCon(_parseFuncDef(_k), &name := 'fName', &params := 'params')
            ; 未做类型检查
            vals := [], vals.length := params.Count
            for k, v in params.OwnProps()
              vals[v] := (k[1] = fnc) ? _doRef(k.substring(2)) : k
            if MeowConf.buildinFunc.Has(name) {
              s .= MeowConf.buildinFunc.Get(name)(vals*), _idx++
              continue
            } else if MeowConf.buildinFunc.Has(nn := SubStr(name, 1, InStr(name, '$')) '*') {
              _s := MeowConf.buildinFunc.Get(nn)(vals*)
              if !IsString(_s) ; 如果返回值是字符串，则可以连接；否则直接返回
                return _s
              s .= _s, _idx++
              continue
            } else if _has(name) {
              ...
              continue
            } else ThrowErr('未定义的函数:' name, _idx, _l, cp)
          }
          _v := _doRef(_k)
          if !IsPrimitive(_v) {
      ...
      _handleMutliLine(_l) {
        local i := 1, inQ := esc := false, cs := _l.toChs()
        while i <= cs.Length {
          esc := false
          if cs[i] = ec
            esc := true, i++
          else if !inQ and cs[i] = A_Space {
            _i := i, _go(cs, A_Space, &i)
            if i <= cs.Length and cs[i] = '\' {
              return _l.substring(1, _i) _handleMutliLine(LTrim(f[r++]))
            }
            return _l
          } else if !esc and cs[i] = this.q
            inQ := !inQ
          i++
        }
        return _l
    ...
    ThrowErr(msg, _c, _l, _f, _r := r - 1) {
      throw Error(JoinStr('', msg, '`n异常文件:', _f, '`n[行' _r, '列' _c ']', _l))
    }

    Warn(msg, _c, _l, _f, _r := r - 1) => (
