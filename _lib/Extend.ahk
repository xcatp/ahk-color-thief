...
ToString(o, q := false, esc := false, expandLevel := unset, space := '  ') {
  ...
  _convert(o, el := 0, l := 0, _ := '') {
    ...
    } else if IsFunction(o) || o is Gui.Control { ; 过滤不需要的对象 
      s := (!l ? '{' : '') nc _getIndent(l + 2) '[' Type(o) ']'
    } else {
      s := !l ? '{' : ''
...
IsFullChar(char) => (_c := '0x' Ord(char).toBase(16)) >= 0xFF01 && _c <= 0xFF5E

MToString(o) => MsgBox(ToString(o))
MTToString(o) => MsgBox(T_(ToString(o)))

; ahk中，字符 '0' 被认为是 false ，甚至 '000000'
Ternary(v, e) => IsTrue(v) ? v : e
